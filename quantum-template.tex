...
\documentclass[a4paper,twocolumn,twoside,superscriptaddress,11pt,accepted=true]{quantumarticle}
\def\authorlist{del Rio, Gogolin, and Huber}
\def\shorttitle{Template demonstrating the quantumarticle class}
\def\acceptdate{{February 30, 2017}}
\pdfoutput=1
...
By passing the option \texttt{accepted=true} the date field will be replaced with the date of acceptance in Quantum. You must define the acceptance date in \texttt{\textbackslash{}acceptdate}, e.g.\ \texttt{\textbackslash{}def\textbackslash{}acceptdate\{\{May 37, 2012\}\}}. 
Additionally, passing the \texttt{twoside} option enables a header with a short title and author list on alternating pages. 
These must be defined in \texttt{\textbackslash{}shorttitle} and \texttt{\textbackslash{}authorlist} via \texttt{\textbackslash{}def}.
...
\pdfoutput=1
\end{verbatim}
within the first within the first 5 lines of the preamble of your main LaTeX file as suggested by the arXiv \cite{arxivpdfoutput}
